...
When IO is interrupted, led_toggle_timeout_handler function is called. 
...
#define LED_TOGGLE_INTERVAL         APP_TIMER_TICKS(1) //发送的间隔时间设置，单位ms              // sending interval setting, unit ms 
//In Transient Case, we set LED_TOGGLE_INTERVAL about 1 as a timer tick. Because there is only one task to do, the total workload is smaller than the other case.
...
//  这里是模拟采集了温度数据，也可以换成其它自定义的函数
//  it is call for simulating temperature sensor ，which can be replaced by other usr defined function 
				temp=88;
				tx_payload.length = 1;
