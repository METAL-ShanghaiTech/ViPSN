...
#define APP_MINOR_VALUE                 0x001	, 0x00                        /**< Minor value used to identify Beacons. */
...
#define ADV_MEAS_INTERVAL      APP_TIMER_TICKS(1000, APP_TIMER_PRESCALER)  /**< Battery level measurement interval (ticks). */
...
static void adv_timeout_handler(void * p_context){
    ...
    //NRF_LOG_INFO("celc: "NRF_LOG_FLOAT_MARKER"\n", NRF_LOG_FLOAT(celciusX100));
	// 修改major和minor
    manuf_specific_data.company_identifier = APP_COMPANY_IDENTIFIER;
    manuf_specific_data.data.p_data = (uint8_t *) m_beacon_info;
    manuf_specific_data.data.size   = APP_BEACON_INFO_LENGTH;
    // modify m_beacon_info
    m_beacon_info[20] = (uint8_t)(celciusX100/100);
    m_beacon_info[21] = (uint8_t)(celciusX100%100);
    // modify advdat
    //memset(&advdata, 0, sizeof(advdata));
		// 广播数据修改
    //advdata.name_type             = BLE_ADVDATA_NO_NAME;
...
//											} 
          ...
          err_code = app_timer_start(m_adv_timer_id, ADV_MEAS_INTERVAL, NULL);
		    advertising_start();
                       buttonMode = 0;
                       break;
                   default:
